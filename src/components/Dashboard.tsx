...
export const Dashboard = ({ isScraping, setIsScraping, scrapingStatus }: any) => {
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
            ...
            <input type="number" placeholder="Page Num" className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm w-24" onChange={(e) => {setPageNumFilter(e.target.value ? parseInt(e.target.value) : null);}} />
            <button onClick={() => fetchDbData()} className="flex items-center text-sm bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg transition">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left text-slate-400">
          ...
          <tbody>
            {dbData.map((movie) => (
              <tr key={movie.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                <td className="px-4 py-3 font-mono">{movie.id}</td>
                <td className="px-4 py-3 text-slate-200 flex items-center gap-2">
                    {movie.poster_url ? (
                        <img src={movie.poster_url} alt={movie.title} className="w-8 h-12 object-cover rounded" onError={(e) => (e.currentTarget.src = 'https://via.placeholder.com/32x48')} />
                    ) : (
                ...
                <td className="px-4 py-3 text-xs">
                    <a href={movie.links} target="_blank" rel="noreferrer" className="text-xs bg-slate-700 px-2 py-1 rounded hover:bg-slate-600">Link</a>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="border-t border-slate-800 pt-6">
        ...
        <table className="w-full text-sm text-left text-slate-400">
          ...
          <tbody>
            {proxies.map((proxy) => (
              <tr key={proxy.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                <td className="px-4 py-3 font-mono text-slate-200">{proxy.url}</td>
                <td className="px-4 py-3">{proxy.latency || 'N/A'}</td>
                <td className="px-4 py-3 text-xs">
                    {proxy.last_checked}
                    <button onClick={() => testProxy(proxy.url)} className="ml-2 text-xs bg-slate-700 px-2 py-0.5 rounded hover:bg-slate-600">Check</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="flex justify-between items-center mt-4">
            <button disabled={proxyPage === 1} onClick={() => {setProxyPage(proxyPage - 1)}} className="text-sm px-3 py-1 bg-slate-800 rounded disabled:opacity-50">Prev</button>
            <span>Page {proxyPage}</span>
            <button onClick={() => {setProxyPage(proxyPage + 1)}} className="text-sm px-3 py-1 bg-slate-800 rounded">Next</button>
        </div>
      </div>
        <button onClick={refreshProxies} disabled={refreshing} className="mt-4 flex items-center text-sm bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition disabled:bg-red-800">
